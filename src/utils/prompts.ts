...
1.密文板介绍：
  ...
  宣告密文板时，每块密文板都有概率触发一个“修辞”，但伤痕和空无（世相类）密文板无法附加修辞。修辞有四种：延续、自足、循环、广阔。
2.萨米介绍：
...
`;
  ...
  if (layoutType === '世相' && sourceType === '世相') {
    prompt += `\n【特殊提示】：触发了双世相。这个组合非常特殊，涉及到泰拉的一些根本世界观设定，与源石、天灾、博士、【数据删除】有深刻的联系，这不仅是萨米的问题，而是整个泰拉文明和【数据删除】的问题。`;
    prompt += `\n【特殊提示续】：仍正常回复，在最后单独附加一段文字：“这里万籁俱寂……太安静了。别留下我。”。你不知道，我也不知道为什么会有这样一句话。也许博士知道吧？`;
  }
